package cc.ixcc.novelthree.jsReader.page;
...
import cc.ixcc.novelthree.ad.CanvasImageButton;
...
import cc.ixcc.novelthree.jsReader.utils.Constant;
import cc.ixcc.novelthree.jsReader.utils.IOUtils;
import cc.ixcc.novelthree.jsReader.utils.RxUtils;
import cc.ixcc.novelthree.jsReader.utils.ScreenUtils;
...
public abstract class PageLoader {
    ...
    /*****************************init params*******************************/
    public PageLoader(PageView pageView, CollBookBean collBook, int startChapter) {
        mPageView = pageView;
        mContext = pageView.getContext();
        mCollBook = collBook;
        mChapterList = new ArrayList<>(1);
        mStartChapter = startChapter;
        // 初始化数据
        initData();
        // 初始化画笔
        initPaint();
    ...
    private void initData() {
        ...
        setUpSpacingParams(mSettingManager.getSpacingSize());
    }
     ...
     * 作用 : 设置行间距
     */
    private void setUpSpacingParams(int spacingSize) {
        // 行间距(大小为字体的一半)
        mTextInterval = spacingSize;
        mTitleInterval = spacingSize;
        // 段落间距(大小为字体的高度)
        mTextPara = spacingSize * 2;
        mTitlePara = spacingSize * 2;
    }
    ...
    private void initPaint() {
        ...
        // 绘制付费的画笔
        mPayPaint = new TextPaint();
        mPayPaint.setColor(mTextColor);
        mPayPaint.setTextSize(ScreenUtils.spToPx(20));
    ...
    public void setSpacingSize(int spacingSize) {
        // 设置行距
        setUpSpacingParams(spacingSize);
        // 取消缓存
        mPrePageList = null;
        mNextPageList = null;
        // 如果当前已经显示数据
    ...
    public void saveRecord() {
        ...
        addHistory(mCollBook.get_id(), (mCurChapterPos + 1) + "");
    }
    ...
    private List<TxtPage> loadPageList(int chapterPos) throws Exception {
        ...
        List<TxtPage> chapters = loadPages(chapter, reader, chapterPos);
        return chapters;
    }
     ...
     * @param b
     */
    public abstract void refreshChapterList(boolean b);
    ...
    private void drawBackground(Bitmap bitmap, boolean isUpdate) {
        ...
        //顶部title距离
        int tipMarginHeight = ScreenUtils.dpToPx(20);
        if (!isUpdate) {
            /****绘制背景****/
            ...
            if (!mChapterList.isEmpty()) {
                ...
                float tipTop;
                if (mPageMode == PageMode.SCROLL) {
                    tipTop = -mTipPaint.getFontMetrics().top;
                } else {
                ...
                /******绘制页码********/
                // 底部的字显示的位置Y
                float y = mDisplayHeight - mTipPaint.getFontMetrics().bottom;
                // 只有finish的时候采用页码
                if (mStatus == STATUS_FINISH) {
        ...
        int visibleBottom = mDisplayHeight;
    ...
    public Boolean isMinutesFree() {
        long saveTime = SharedPreUtils.getInstance().getLong(ReadSettingManager.SAVE_FREE_AD_TIME, 0);
        if (saveTime == 0) {
            return false;
        }

        long NowMinites = System.currentTimeMillis() / 1000;
        Log.e("Free Ad Time test", "saveTime:" + saveTime + ",NowMinites:" + NowMinites);
        if (NowMinites > saveTime) {
            SharedPreUtils.getInstance().putLong(ReadSettingManager.SAVE_FREE_AD_TIME, 0);
            return false;
        }
    ...
    private CanvasImageButton canvasImageButton;
    private CanvasImageButton canvasImageButton1;
    private List<Rect> mClickRects =new ArrayList<>();

    //TODO 添加绘制广告
    private void drawContent(Bitmap bitmap) {
        mPageCanvas = new Canvas(bitmap);

        if (mPageMode == PageMode.SCROLL) {
            mPageCanvas.drawColor(mBgColor);
        }
        ...
        if (mStatus != STATUS_FINISH) {
            ...
            switch (mStatus) {
                case STATUS_LOADING:
                    ...
                    break;
                case STATUS_ERROR:
                    tip = "Failed to load (click the edge to retry)";
                    break;
                case STATUS_EMPTY:
                    tip = "The article content is empty";
                    break;
                case STATUS_PARING:
                    tip = "Typesetting, please wait...";
                    break;
                case STATUS_PARSE_ERROR:
                    tip = "File parsing error";
                    break;
                case STATUS_CATEGORY_EMPTY:
                    tip = "Directory list is empty";
                    break;
            }
            ...
            mPageCanvas.drawText(tip, pivotX, pivotY, mTextPaint);
        } else {
            if (!mCurPage.hasPay) {
                ...
                if (mPageMode == PageMode.SCROLL) {
                    ...
                    top = mMarginHeight + ScreenUtils.dpToPx(20) - mTextPaint.getFontMetrics().top;
                }
                ...
                for (int i = 0; i < mCurPage.titleLines; ++i) {
                    ...
                    //进行绘制
                    mPageCanvas.drawText(str, start, top, mTitlePaint);
                ...
                for (int i = mCurPage.titleLines; i < mCurPage.lines.size(); ++i) {
                    ...
                    mPageCanvas.drawText(str, mMarginWidth, top, mTextPaint);
                    if (str.endsWith("\n")) {
                        top += para;
                    } else {
                        top += interval;
                ...
                //如果不是VIP且当前页为最后一页
//                if (!checkVip() && mCurPage.position + 1 == mCurPageList.size()) {
//                    Log.d(TAG, "drawContent: "+mCurPage.lines.size()+"|"+mCurPage.titleLines);
//                    //当前页内容不超过18行,才显示广告内容
//                    if (mCurPage.lines.size() <= 18) {
//
//                        Paint paint = new Paint();
//                        //绘制激励视频点击按钮
//                        canvasImageButton = new CanvasImageButton(mContext, R.drawable.ad);
//                        canvasImageButton.DrawImageButton(mPageCanvas, (mPageCanvas.getWidth() - canvasImageButton.mWidth) / 2, (int) top, paint);
//                        //绘制VIP充值引导按钮
//                        canvasImageButton1 = new CanvasImageButton(mContext, R.drawable.ad1);
//                        canvasImageButton1.DrawImageButton(mPageCanvas, (mPageCanvas.getWidth() - canvasImageButton1.mWidth) / 2, mPageCanvas.getHeight()-canvasImageButton1.mHeight - ScreenUtils.dpToPx(60), paint);
//
//                        mClickRects.clear();
//                        mClickRects.add(canvasImageButton.GetRect());
//                        mClickRects.add(canvasImageButton1.GetRect());
//                        //设置按钮点击事件
//                        mPageView.setClickListener(new PageView.ClickListener()
//                        {
//                            @Override
//                            public void click(int index) {
//                                //0 点击广告播放按钮 1 点击VIP充值按钮
//                                switch (index){
//                                    case 0:
...
//                                                //激励视频激励成功
//                                                AdMobManager.GetInstance().GetCoin("15",1);
//                                            }
//                                        });
...
//                                            ThirdLoginActivity.start(mContext, ThirdLoginActivity.EnterIndex.PAY);
//                                        } else {
//                                            OpenVipActivity.start(mContext);
//                                        }
//                                        break;
//                                    default:
//                                        break;
...
//                }
                ...
                //对内容进行绘制
                for (int i = mCurPage.titleLines; i < mCurPage.lines.size(); ++i) {
                    if (i >= mCurPage.lines.size() - mCurPage.titleLines - 4) {
                        break;
                    }
                    str = mCurPage.lines.get(i);
                    mPageCanvas.drawText(str, mMarginWidth, top, mTextPaint);
                ...
                float textHeight = fontMetrics.top - fontMetrics.bottom;
                float textWidth = mPayPaint.measureText("This chapter requires paid reading");
                float pivotX = (mDisplayWidth - textWidth) / 2;
                float pivotY = (mDisplayHeight - textHeight) / 2;
                pivotY = Math.max(pivotY, top);
                pivotY = Math.max(pivotY, mDisplayHeight * 4f / 5);
                mPageCanvas.drawText("This chapter requires paid reading", pivotX, pivotY, mPayPaint);
                mPageChangeListener.onPayPage(mCollBook, mCurChapterPos);
    ...
    /**************************************private method********************************************/
    //加载页面数据
    private List<TxtPage> loadPages(TxtChapter chapter, BufferedReader br, int chapterPos) {
        if (checkVip() || mChapterList.get(chapterPos).getCoin() <= 0 || mChapterList.get(chapterPos).isIs_pay()) {
            //生成的页面
                ...
                while (showTitle || (paragraph = br.readLine()) != null) {
                    ...
                    while (paragraph.length() > 0) {
                        ...
                        if (wordCount < paragraph.length()) {
                            String tempSubString = paragraph.substring(0, wordCount);
                            wordCount = getChangeCount(paragraph, wordCount);
                        }
    ...
    public static int getChangeCount(String paragraph, int wordCount) {
        for (int i = wordCount; i > 0; i--) {
            int pre = paragraph.charAt(wordCount - 1);
            int next = paragraph.charAt(wordCount);

            //如果这两位都是字符，说明是一个单词里面的
            if (isLetter(pre) && isLetter(next)) {
    ...
    public static boolean isLetter(int num) {
    ...
    public interface OnPageChangeListener {
        ...
        void onPayPage(CollBookBean mCollBook, int mCurChapterPos);
    }
