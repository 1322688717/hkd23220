package cc.ixcc.novelthree.jsReader.animation;
...
public class ScrollPageAnim extends PageAnimation {
    ...
    private int topMargin = ScreenUtils.dpToPx(20);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                isRunning = true;
                mListener.onRunning(true);
                // 进行刷新
                mView.postInvalidate();
                break;
            case MotionEvent.ACTION_UP:
                isRunning = false;
                mListener.onRunning(false);
                // 开启动画
                startAnim();
                // 删除检测器
                mVelocity.recycle();
...
/*        //设置背景透明
    ...
    public Bitmap getNextBitmap() {
        try {
            return mScrapViews.getFirst().bitmap;
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
...
//        try {
//            mScrapViews.getFirst().bitmap = bitmap;
//        } catch (Exception e) {
//            e.printStackTrace();
//        }
//    }
