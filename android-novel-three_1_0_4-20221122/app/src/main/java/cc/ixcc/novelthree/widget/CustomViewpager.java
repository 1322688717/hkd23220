...
public class CustomViewpager extends ViewPager {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        if (mChildrenViews.size() > current) {
            ...
            height = child.getMeasuredHeight() + DpUtil.dip2px(getContext(), 67);
            if (height < DisplayUtils.getScreenHeightPixels((AppCompatActivity) getContext())) {
                height = DisplayUtils.getScreenHeightPixels((AppCompatActivity) getContext());
            }
