...
public class InviteCodeDialog extends Dialog {
    ...
    static InviteCodeDialog myDialog = null;
    DialogCallBack mDialogCallBack;

    public InviteCodeDialog(Context context, int theme) {
        super(context, theme);
    }
     ...
     */
    public static InviteCodeDialog getMyDialog(Activity context) {
        mContext = context;
        myDialog = new InviteCodeDialog(context, R.style.dialog);// 创建自定义样式dialog
        myDialog.setContentView(R.layout.dialog_invite_code);// 得到加载view
...
//        myDialog.getWindow().getAttributes().gravity = Gravity.BOTTOM;//设置显示位置为下方
     ...
     * @return
     */
    public InviteCodeDialog setMessage(String strMessage) {
