...
import cc.ixcc.novelthree.interfaces.ActivityResultCallback;
...
//                        uri = FileProvider.getUriForFile(mContext, WordUtil.getString(R.string.FILE_PROVIDER), mCameraResult);
                        uri = FileProvider.getUriForFile(mContext, BuildConfig.applicationId+".fileprovider", mCameraResult);
                    } else {
...
//            uri = FileProvider.getUriForFile(mContext, WordUtil.getString(R.string.FILE_PROVIDER), mCameraResult);
    ...
    private File getNewFile() {
        // 裁剪头像的绝对路径
        File dir = new File(Constants.CAMERA_IMAGE_PATH);
        if (!dir.exists()) {
            dir.mkdirs();
        }
...
//                Uri resultUri = FileProvider.getUriForFile(mContext, WordUtil.getString(R.string.FILE_PROVIDER), mCorpResult);
                Uri resultUri = FileProvider.getUriForFile(mContext, BuildConfig.applicationId + ".fileprovider", mCorpResult);
                if (resultUri == null || mFragment == null || mContext == null) {
                    return;
                }
                UCrop uCrop = UCrop.of(inputUri, resultUri)
