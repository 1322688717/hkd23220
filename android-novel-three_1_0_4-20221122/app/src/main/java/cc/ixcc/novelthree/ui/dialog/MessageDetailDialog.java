...
public class MessageDetailDialog extends Dialog {
    ...
    static MessageDetailDialog myDialog = null;
    DialogCallBack mDialogCallBack;
    TextView updateTime;
    TextView readTime;
    TextView cancel;

    public MessageDetailDialog(Context context, int theme) {
        super(context, theme);
    }
     ...
     */
    public static MessageDetailDialog getMyDialog(Activity context) {
        mContext = context;
        myDialog = new MessageDetailDialog(context, R.style.dialog2);// 创建自定义样式dialog
        myDialog.setContentView(R.layout.dialog_message_detail);// 得到加载view
...
//        myDialog.getWindow().getAttributes().gravity = Gravity.CENTER;//设置显示位置为中间
    ...
    private void initUI() {
        updateTime = myDialog.findViewById(R.id.title);
        readTime = myDialog.findViewById(R.id.title2);
     ...
     * @return
     */
    public MessageDetailDialog setMessage(String strMessage) {
