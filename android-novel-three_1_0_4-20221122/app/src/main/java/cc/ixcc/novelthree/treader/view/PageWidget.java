package cc.ixcc.novelthree.treader.view;
...
import cc.ixcc.novelthree.treader.view.animation.AnimationProvider;
import cc.ixcc.novelthree.treader.view.animation.CoverAnimation;
import cc.ixcc.novelthree.treader.view.animation.NoneAnimation;
import cc.ixcc.novelthree.treader.view.animation.SimulationAnimation;
import cc.ixcc.novelthree.treader.view.animation.SlideAnimation;
import cc.ixcc.novelthree.treader.view.animation.UpDownSlideAnimation;
...
public class PageWidget extends View {
    ...
    Bitmap mNextPageBitmap = null; // 需要转到页 可能是上一页 也可能是下一页
    private AnimationProvider mAnimationProvider;

    Scroller mScroller;
    //    private int mBgColor = 0xFFCEC29C;
    private int mBgColor = 0x000000;
    ...
    public void setPageMode(int pageMode) {
        switch (pageMode) {
            ...
            case Config.PAGE_MODE_SLIDE:
                ...
                break;
            case Config.PAGE_MODE_UPDOWN:
                mAnimationProvider = new UpDownSlideAnimation(mCurPageBitmap, mNextPageBitmap, mScreenWidth, mScreenHeight);
                break;
            case Config.PAGE_MODE_NONE:
                mAnimationProvider = new NoneAnimation(mCurPageBitmap, mNextPageBitmap, mScreenWidth, mScreenHeight + mScreenHeight);
                break;
            default:
//                mAnimationProvider = new SimulationAnimation(mCurPageBitmap, mNextPageBitmap, mScreenWidth, mScreenHeight);
                mAnimationProvider = new CoverAnimation(mCurPageBitmap, mNextPageBitmap, mScreenWidth, mScreenHeight);
...
//            cancelPage = false;
            ...
            return pageTmoveHandle(x, y, event);
...
//                if (downX > mScreenWidth / 5 && downX < mScreenWidth * 4 / 5 && downY > mScreenHeight / 3 && downY < mScreenHeight * 2 / 3) {
                if (downX > mScreenWidth * 2 / 5 && downX < mScreenWidth * 3 / 5 && downY > mScreenHeight / 3 && downY < mScreenHeight * 2 / 3) { //缩小中间点击范围
...
//            if(this.pageMode == Config.PAGE_MODE_UPDOWN)  mAnimationProvider.onRelaseUp();
    ...
    private boolean pageUpHandle(int x, int y, MotionEvent event) {
        if (!isMove) {
            switch (pageMode) {
                case Config.PAGE_MODE_SIMULATION:
...
//                  break;
    ...
    private boolean pageTmoveHandle(int x, int y, MotionEvent event) {
//        if (isNext) {
//            Boolean isNext = mTouchListener.nextPage();
//            if (!isNext) {
//                return true;
...
//            }
//        }
        // if (Config.getInstance().getPageMode() == Config.PAGE_MODE_NONE) return false;
        final int slop = ViewConfiguration.get(getContext()).getScaledTouchSlop();
//        Log.e(TAG, "slop: "+ slop);
        //判断是否移动了
        if (!isMove) {
            isMove = Math.abs(downX - x) > slop || Math.abs(downY - y) > slop;
//            isMove = Math.abs(downY - y) > slop;
        }
        if (isMove) {
            if (moveX == 0 && moveY == 0) {
                Log.e(TAG, "isMove");
                //判断翻得是上一页还是下一页
//                if (Config.getInstance().getPageMode() == Config.PAGE_MODE_UPDOWN) {
//                    if (y - downY > 0) {
//                        isNext = false;
//                    } else {
//                        isNext = true;
